...
const AddRatesList: React.FC = () => {
  ...
  const [users, setUsers] = useState<ProductRow[]>([
    {
  ...
  const fetchSubCategories = useCallback(
    async (categoryName: string): Promise<SubCategoryOption[]> => {
      if (!categoryName) {
        setSubCategory([]);
        return [];
      }
      try {
        ...
        const list = response?.data?.categorySub ?? [];
        setSubCategory(list);
        return list;
      } catch (error: any) {
  ...
  const handleTraderChange = (e: React.ChangeEvent<HTMLSelectElement>): void => {
